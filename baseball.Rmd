...
```{r, cache=TRUE}
set.seed(139)
lmer.varmodel <- lmer(W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + H9 + (1 + WHIP + W.L..same_year + Age.pitch + H9 | Tm), data = train.df)
# summary(lmer.varmodel)
# predict(lmer.varmodel)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
...
```{r, cache=TRUE}
set.seed(139)
lmer.varmodel <- lmer(W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + tot_fa_war3 + IBB.bat + (1 + W.L..same_year + Age.pitch + tot_fa_war3 + IBB.bat | Tm), data = train.df)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
```
...
```{r, cache=TRUE}
set.seed(139)
lmer.varmodel <- lmer(W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + tot_fa_war3 + IBB.bat + (1 | Tm), data = train.df)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
```
...
```{r, cache=TRUE}
set.seed(139)
# lmer.varmodel <- lmer(W.L..next_year ~ Age.bat + PA + AB + H.bat + X2B + X3B + HR.bat + SB + CS + BB.bat + SO.bat + BA + OBP + SLG + OPS + OPSplus + TB + GDP + HBP.bat + SH + SF + IBB.bat + Age.pitch + W.L..same_year + GF + SHO + SV + IP + H.pitch + HR.pitch + BB.pitch + IBB.pitch + SO.pitch + HBP.pitch + BK + WP + BF + FIP + WHIP + H9 + HR9 + BB9 + SO9 + SO.W +  G + Inn + Ch + PO + A + E + DP + Fld. + RF.9 + RF.G + tot_fa_war3 + num_fas || Tm , data = train.df)
# RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
# RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
```