...
baseline.adjrsq = summary(baseline)$adj.r.squared
```
...
```{r}
lmer.varmodel <- lmer(W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + (1 + WHIP + W.L..same_year + Age.pitch | Tm), data = train.df)
...
```{r}
lmer.varmodel <- lmer(W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + H9 + (1 + W.L..next_year ~ WHIP + W.L..same_year + Age.pitch + H9 | Tm), data = train.df)
# summary(lmer.varmodel)
# predict(lmer.varmodel)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
