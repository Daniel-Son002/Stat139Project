...
```{r}
set.seed(139)
lmer.varmodel <- lmer(W.L..next_year ~ OBP + Fld. + BA + (1 + OBP + Fld. + BA | Tm) , data = train.df)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
```
...
```{r}
set.seed(139)
lmer.varmodel <- lmer(W.L..next_year ~ Fld. + OBP:Fld. + BA:OBP + (1 + Fld. + OBP:Fld. + BA:OBP | Tm) , data = train.df)
RMSE(train.df$W.L..next_year, predict(lmer.varmodel))
RMSE(test.df$W.L..next_year, predict(lmer.varmodel, newdata=test.df))
```
